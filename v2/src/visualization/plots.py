"""Original plots generated from V2 experiment records."""
...
def plot_training_curves(
    ...
    axis.set(title="V2 Phase 1 baseline loss", xlabel="Epoch", ylabel="Cross-entropy loss")
    axis.legend()
    axis.grid(alpha=0.2)
    figure.tight_layout()
    ...
    figure, axis = plt.subplots(figsize=(8, 5))
    axis.plot(
        epochs, [row["train_accuracy"] for row in history], label="Train accuracy"
    )
    axis.plot(
        epochs,
    ...
    axis.set(title="V2 Phase 1 baseline accuracy", xlabel="Epoch", ylabel="Accuracy")
    axis.legend()
    axis.grid(alpha=0.2)
    figure.tight_layout()
...
def plot_confusion_matrix(
    matrix: np.ndarray, class_names: Sequence[str], output_path: str | Path
) -> None:
    ...
    axis.set(
        ...
        title="V2 Phase 1 baseline: frozen V1 test split",
        ylabel="Actual class",
